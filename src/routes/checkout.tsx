    ...
    <section className="container-x py-12">
      ...
      <form ref={formRef} onSubmit={placeOrder} className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Payment - 3 Options Clearly Visible */}
          <div className="bg-card rounded-2xl p-6 shadow-card">
            <h2 className="font-display text-xl text-primary mb-4">Select Payment Method</h2>
            <div className="space-y-3">
              
              {/* Option 1: COD */}
              <label
                className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-colors ${method === "cod" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
              >
                <input
                  type="radio"
                  name="pay"
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Pay when you receive the order at your doorstep</p>
                </div>
              </label>
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <p className="font-semibold text-lg text-green-800">📱 Scan QR Code (UPI)</p>
              ...
              <label
                className={`flex items-start gap-3 p-4 rounded-xl border cursor-pointer transition-colors ${method === "razorpay" ? "border-primary bg-primary/5" : "border-border hover:border-primary/50"}`}
              >
                <input
                  type="radio"
                  name="pay"
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Credit/Debit Cards • Net Banking • UPI AutoPay • Wallets</p>
                </div>
              </label>
            </div>
            <p className="flex items-center gap-2 text-xs text-muted-foreground mt-4">
              <Lock className="w-3.5 h-3.5" /> 100% Secure · SSL Encrypted · PCI-DSS Compliant
            </p>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="bg-gradient-to-r from-green-600 to-emerald-500 px-6 py-5 text-white text-center">
              ...
              <p className="text-sm opacity-90 mt-1">Scan & pay using any UPI app</p>
            </div>
            ...
            <div className="p-6">
              ...
              <div className="bg-gray-50 rounded-2xl p-4 border text-center">
                ...
                />
                <div className="mt-4">
                  <p className="text-lg font-bold text-green-700">Amount: {inr(total)}</p>
                  <p className="text-sm text-gray-500 mt-1">Scan this QR using Google Pay, PhonePe, Paytm or any UPI app</p>
                </div>
              </div>

              {/* UPI ID */}
              <div className="mt-5 bg-green-50 border border-green-200 rounded-xl p-4">
                <p className="text-sm text-gray-600 mb-2 font-medium">Manual UPI ID (Copy & Pay)</p>
              ...
              {/* UPI Apps Shortcut */}
              <div className="mt-5">
                <p className="text-sm text-gray-600 mb-3">Or open directly in:</p>
                <div className="flex gap-3 justify-center">
                  <button onClick={() => openUPIApp("gpay")} className="px-4 py-2 bg-blue-500 text-white rounded-xl text-sm">Google Pay</button>
                  <button onClick={() => openUPIApp("phonepe")} className="px-4 py-2 bg-purple-500 text-white rounded-xl text-sm">PhonePe</button>
  ...
  items: items.map((it) => ({
    ...
    price: it.price,
              ...
              <button
                ...
                className="w-full mt-6 px-6 py-3 bg-green-600 text-white rounded-xl font-semibold hover:bg-green-700 transition"
              >
                I have completed the payment
              </button>

              <p className="text-xs text-gray-500 text-center mt-4">
                After payment, click above to confirm your order
              </p>
            </div>
          </div>
        </div>
