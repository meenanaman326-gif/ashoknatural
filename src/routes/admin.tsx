...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [{ title: "Admin Dashboard — Ashok Naturals" }],
  }),
  component: Admin,
});
  ...
  return (
    <section className="container-x py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          <p className="text-[10px] tracking-[0.4em] uppercase text-gold mb-1">
            Admin Panel
          </p>
  ...
  return (
    <div className="bg-card rounded-2xl p-5 shadow-card">
      <div className="flex items-center justify-between mb-2">
        <div className="w-10 h-10 rounded-xl bg-gradient-gold grid place-items-center">
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-muted-foreground">
                {p.category} · {p.weights.length} variants
              </p>
            </div>

            <span className="font-display text-primary">
              {inr(p.weights[0].price)}
            </span>
  ...
  return (
    <div className="bg-card rounded-2xl p-6 shadow-card">
      ...
      <p className="text-sm text-muted-foreground">
        Manage your customer database, view order history,
        and segment by purchase behavior.
      </p>
    </div>
  );
}
...
function BannersView() {
  return (
    <div className="bg-card rounded-2xl p-6 shadow-card">
      ...
      <p className="text-sm text-muted-foreground">
        Upload and re-order hero banners and promotional
        carousels.
      </p>
    </div>
  );
...
function TestimonialsView() {
  return (
    <div className="bg-card rounded-2xl p-6 shadow-card">
      ...
      <p className="text-sm text-muted-foreground">
        Approve and showcase customer reviews on the homepage.
      </p>
    </div>
  );
}
        ...
        {products.map((p) => (
          ...
          >
            ...
            <span className="text-sm text-muted-foreground">
              {Math.floor(Math.random() * 200 + 20)} units
            </span>
          </div>
        ))}
      </div>
...
function InvoicesView() {
  return (
    <div className="bg-card rounded-2xl p-6 shadow-card">
      ...
      <p className="text-sm text-muted-foreground">
        Auto-generated GST-compliant invoices for every
        order. Download as PDF or email to customers.
      </p>
    </div>
  );
            }
