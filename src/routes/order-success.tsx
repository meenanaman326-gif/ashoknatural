  ...
  return (
    <section className="container-x py-20 max-w-2xl mx-auto text-center">
      ...
      <p className="text-[10px] tracking-[0.4em] uppercase text-gold mb-2">
        {isFailed ? "Payment failed" : isProcessing ? "Payment processing" : "Order confirmed"}
      </p>
      ...
      <p className="text-muted-foreground mt-3">
        {isFailed
          ...
          : isProcessing
            ? "We're checking the final payment status. This usually takes a few seconds."
      ...
      <div className="bg-card rounded-2xl p-6 shadow-card mt-8 text-left">
        <div className="flex items-center justify-between border-b pb-4 mb-4">
          ...
          <span className="px-3 py-1 rounded-full bg-gold/20 text-primary text-xs font-semibold">
            {isFailed ? "Failed" : isProcessing ? "Processing" : "Confirmed"}
          </span>
        </div>
        <div className="space-y-3">
          {[
            {
              icon: CheckCircle2,
              t: isFailed ? "Payment retry needed" : "Order placed",
              d: isFailed
                ? "No amount was captured"
                : isProcessing
                  ? "Awaiting payment confirmation"
                  : "We've received your order",
              done: !isFailed && !isProcessing,
            },
            ...
            <div key={s.t} className="flex items-center gap-3">
              <div
                className={`w-9 h-9 rounded-full grid place-items-center ${s.done ? "bg-gradient-gold text-primary" : "bg-muted text-muted-foreground"}`}
              >
      ...
      <div className="flex gap-3 justify-center mt-8">
        ...
        <Link to="/auth" className="px-6 py-3 rounded-full border border-border font-semibold">
          Track order
        </Link>
      </div>
    </section>
